...
static char * testHashInit() {
  ...
  muAssert("error, hash size unexpected", hash.size == hashSize);
...
static char * testHashInsert() {
  ...
  muAssert("error, first value cuarenta", hash.data[0].elements[0].value==40);
  muAssert("error, second value cincuenta", hash.data[0].elements[1].value==50);
  return 0;
}
...
static char * testHashGet() {
  ...
  muAssert("error, novalue must be 100-", noValue == NULL);
  return 0;
}
